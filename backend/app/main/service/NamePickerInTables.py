...
class NamePickerInTables():
    ...
    def getAllNames(self, threshold:float) -> list:
        return list(chain.from_iterable([
                        dataName['names'] for (_,dataName) in self.picker.items() if len(dataName['names']) > 0 and dataName["countOfRealName"] / len(dataName['names']) > threshold
                    ])
                )
